...
class Table:
    ...
    def write_to_disk(self):
        # arbitrary record location, just for testing
        base_record_location = self.page_directory[1]
        pagerangeid = base_record_location[0]
        frame = Frame(self.name, pagerangeid, 0, self.page_ranges[pagerangeid])
        self.bufferpool.write_to_disk(frame)
        # store all table information
        self.bufferpool.store_table_metadata(self.name, self.key, self.num_columns, self.page_directory, self.keyToRID, self.index)
